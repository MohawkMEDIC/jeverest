...
public class CD<T> extends CE<T>  {
    ...
    {
        ArrayList<IResultDetail> retVal = (ArrayList<IResultDetail>)(super.validateEx());
        if (this.getQualifier() != null && this.getCode() == null)
            retVal.add(new DatatypeValidationResultDetail(ResultDetailType.ERROR, "CD", String.format(EverestValidationMessages.MSG_DEPENDENT_VALUE_MISSING, "Qualifier", "Code"), null));
        return retVal;
    }
    
	/**
	 * Extended set Qualifier method for formatting
	 * @param parse
	 */
	public void setQualifier(Iterable<? extends CR> qualifier) {
		this.m_qualifier = new LIST<CR<T>>(qualifier);
	}
}
